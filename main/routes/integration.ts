...
function requireIntegrationKey(req: Request, res: Response): boolean {
  ...
  const validInternal = supplied && timingSafeEqualText(supplied, internalToken);
  const validExternal = externalToken && timingSafeEqualText(supplied, externalToken);
...
function getCatalogRevision(db: ReturnType<typeof getDatabase>): number {
  const row = db.prepare('SELECT revision FROM wordpress_bridge_catalog_state WHERE id = 1').get() as { revision?: number } | undefined;
...
integrationRoutes.get('/catalog/changes', (req, res) => {
  ...
  const changes = db.prepare('SELECT revision, entity_type, entity_id, action, changed_at FROM wordpress_bridge_catalog_changes WHERE revision > ? ORDER BY revision ASC LIMIT 500').all(afterRevision);
...
integrationRoutes.get('/orders/changes', (req, res) => {
  const db = getDatabase();
  const afterRevision = Math.max(0, Number(req.query.after_revision || 0));
  const currentRevision = Number((db.prepare('SELECT revision FROM wordpress_bridge_order_state WHERE id = 1').get() as any)?.revision || 0);
  const changes = db.prepare("SELECT c.revision, c.order_id, o.external_order_id, c.status, c.changed_at FROM wordpress_bridge_order_changes c JOIN orders o ON o.id = c.order_id WHERE c.revision > ? AND o.online_platform = 'wordpress' ORDER BY c.revision ASC LIMIT 500").all(afterRevision);
  res.json({ after_revision: afterRevision, revision: currentRevision, has_more: (changes as any[]).length === 500, changes });
});
